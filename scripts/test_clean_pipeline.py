"""
Test the cleaned-up pipeline using the actual placement data structure.
"""
import sys
import json
...
from ai_agent.placement.finger_grouper import _resolve_row_overlaps, maximize_interleaved_abutment
...
print("=== Testing Cleaned-Up Pipeline ===\n")
print(f"Loaded {len(data['nodes'])} nodes from {placement_file}")

# Use the nodes directly (already expanded fingers)
nodes = data['nodes']
print(f"Processing {len(nodes)} nodes")

# Step 1: Optimize orientations for abutment (should skip interleaved rows)
...
# Step 2: Resolve overlaps and add fillers (includes maximize_interleaved_abutment)
...
if total_gaps == 0:
    ...
    abutment_rate = total_abutments / (total_abutments + total_gaps) * 100
    print(f"Abutment rate: {abutment_rate:.1f}%")

# Save the result
output_file = Path("scratch/clean_pipeline_test.json")
