...
def node_placement_specialist(state):
    ...
    if goals_active:
        ...
        log_detail(f"  Matching={match_priority}  Area={area_priority}  "
    ...
    context_text = build_placement_context(
        nodes, constraint_text, terminal_nets=terminal_nets, edges=edges,
    ...
    try:
        ...
        log_detail(f"Matching priority={match_priority}  area_priority={area_priority}")
