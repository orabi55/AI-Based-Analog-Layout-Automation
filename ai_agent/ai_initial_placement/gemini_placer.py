...
def gemini_generate_placement(input_json: str, output_json: str):
    ...
    # Detect structural matching + enrich with net-based matching
    ...
    # Row assignment with symmetry-aware ordering (using merged nodes)
    ...
    # Build abutment constraint string
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        try:
            ...
            # --- Expand group-level placement to finger-level ----------------
            # Do this BEFORE validation so we validate finger IDs (not group IDs)
            ...
            # HEAL: Algorithmic fix for spacing after expansion
            candidates = graph_data.get("abutment_candidates", [])
            expanded_nodes = _heal_abutment_positions(expanded_nodes, candidates,
                                                       no_abutment=no_abutment)
            
            # FAILSAFE: Force correct abutment spacing if healing didn't work
            if not no_abutment:
                expanded_nodes = _force_abutment_spacing(expanded_nodes, candidates)

            # Validate against original finger-level nodes
            val_errors = _validate_placement(norm_nodes, expanded_nodes)
            if val_errors:
                print(f"[gemini_placer] Validation warnings: {val_errors[:3]}")
                # Don't fail hard on validation — the de-overlap should have fixed overlaps
