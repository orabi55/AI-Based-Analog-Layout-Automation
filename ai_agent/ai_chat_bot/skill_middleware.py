...
skill catalog. Skill bodies are loaded on demand using the load_skill tool.

Follows the AgentMiddleware pattern:
  - On init: scans skills_dir, reads frontmatter only
  - On every model call: appends skill catalog to system prompt via wrap_model_call
