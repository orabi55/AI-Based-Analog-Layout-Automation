...
def node_strategy_selector(state: LayoutState):
    ...
    print("[STRATEGY] Running LLM strategy selection...")
    print(f"[STRATEGY] Prompt:\n{strategy_prompt}")

    try:        
        strategy_response = llm.invoke(strategy_prompt)
        strategy_text = strategy_response.content
        print(f"[STRATEGY] Generated strategies:\n{strategy_text}")
    except Exception as exc:
        print(f"[STRATEGY] LLM failed to generate strategies: {exc}")
        strategy_text = ""
...
def node_placement_specialist(state: LayoutState):
    ...
    if pending_cmds:
        ...
        print(f"[PLACEMENT] Updated nodes:\n{updated_nodes}")
        return {
    ...
    print(f"[PLACEMENT] Running LLM placement specialist")
    print(f"[PLACEMENT] Prompt:\n{placer_msgs}")

    placement_response = ""
    try:
        placement_response = llm.invoke(placer_msgs)
        stage2_cmds = _extract_cmd_blocks(placement_response.content)
    except Exception as exc:
...
def node_drc_critic(state: LayoutState):
    ...
    critic_response = ""
    try:
        critic_response = llm.invoke(critic_msgs)
