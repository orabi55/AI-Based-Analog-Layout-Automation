...
========================
Evaluates placement for estimated wire length and net-crossing complexity.
Provides recommendations for swaps to reduce routing congestion.

Functions:
- _classify_net: Classifies a net as 'critical', 'bias', or 'signal' based on its name.
  - Inputs: net_name (str)
  - Outputs: classification string.
- score_routing: Estimates routing quality from net x-spans and crossing counts.
  - Inputs: nodes (list), edges (list), terminal_nets (dict)
  - Outputs: dict with score, spans, details, and summary.
- format_routing_for_llm: Formats routing analysis results for LLM consumption.
  - Inputs: routing_result (dict), nodes (list), terminal_nets (dict)
