"""
File Description:
This module defines the specialized system prompts for various agents in the multi-agent pipeline, including the Chat, Analyzer, Refiner, Adapter, and Code Generator agents. It also includes utility functions to format layout context and compute grid information.

Functions:
- build_chat_prompt:
    - Role: Constructs a conversational system prompt for the chat assistant.
    - Inputs: 
        - layout_context (dict | None): Current layout metadata.
    - Outputs: (str) Formatted system prompt.
- build_analyzer_prompt:
    - Role: Constructs a prompt for the Analyzer Agent to identify circuit topology and propose layout strategies.
    - Inputs: 
        - layout_context (dict | None): Current layout metadata.
    - Outputs: (str) Specialized analysis prompt.
- build_refiner_prompt:
    - Role: Constructs a prompt for the Solution Refiner to present options to the designer.
    - Inputs: None
    - Outputs: (str) Refinement prompt.
- build_adapter_prompt:
    - Role: Constructs a prompt for the Solution Adapter to map abstract strategies to concrete device IDs.
    - Inputs: 
        - layout_context (dict | None): Current layout metadata.
    - Outputs: (str) Adapter prompt.
- build_codegen_prompt:
    - Role: Constructs a prompt for the Code Generator to produce strict JSON layout commands.
    - Inputs: 
        - layout_context (dict | None): Current layout metadata.
    - Outputs: (str) JSON generation prompt with grid rules.
- _compute_grid_info:
    - Role: Extracts and computes physical grid parameters (pitch, row Y, row pitch) from the layout context.
    - Inputs: 
        - layout_context (dict | None): Current layout metadata.
    - Outputs: (dict) Dictionary of grid parameters.
- _format_layout_context:
    - Role: Formats complex layout data into a compact, human-readable text block for LLM consumption.
    - Inputs: 
        - layout_context (dict | None): Current layout metadata.
    - Outputs: (str) Text representation of the layout state.
"""
...
def build_refiner_prompt() -> str:
    ...
    return (
        ...
        "   you would like to apply, or describe any modifications.'\n"
...
def build_codegen_prompt(layout_context: dict | None) -> str:
    ...
    prompt = (
        ...
        "- Abutting MM_A and MM_B moves them side-by-side (X distance = 0.070um).\n"
