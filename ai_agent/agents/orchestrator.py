...
========================
Drives the multi-agent pipeline for layout automation, coordinating intent 
classification and agent invocation.

Functions:
- reset: Resets the orchestrator state.
- process: Main entry point for processing a conversation turn.
  - Inputs: user_message (str), layout_context (dict), chat_history (list), run_llm_fn (callable), selected_model (str)
  - Outputs: dict with reply, commands, and waiting status.
- _handle_chat: Handles general chat turns.
- _handle_question: Handles specific circuit questions.
- _handle_concrete: Handles requests for concrete layout changes.
- _handle_abstract: Handles high-level strategy requests using a multi-step pipeline.
- _handle_refiner_feedback: Processes user approval for proposed strategies.
"""
...
class MultiAgentOrchestrator:
    ...
    def _handle_concrete(self, msg, ctx, run_llm_fn, model):
        from ai_agent.agents.prompts import build_codegen_prompt
    ...
    def _handle_abstract(self, msg, ctx, run_llm_fn, model):
        ...
        from ai_agent.agents.prompts import (
