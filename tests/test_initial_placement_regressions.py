...
def test_gemini_generate_placement_preserves_input_schema(monkeypatch, tmp_path):
    from ai_agent.ai_initial_placement import gemini_placer as gp
    ...
    class _FakeModels:
        ...
        def generate_content(**kwargs):
            return SimpleNamespace(
                text='{"nodes":[{"id":"MN0","type":"nmos","geometry":{"x":0.0,"y":0.0,"width":0.294,"height":0.668,"orientation":"R0"}}]}'
            )
    ...
    monkeypatch.setattr(gp.genai, "Client", _FakeClient)
    monkeypatch.setattr(
        gp,
        "types",
    ...
    gp.gemini_generate_placement(str(input_path), str(output_path))
