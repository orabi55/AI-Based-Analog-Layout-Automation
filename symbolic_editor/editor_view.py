...
from PySide6.QtGui import QPainter, QPen, QPainterPath, QColor, QBrush, QAction, QKeySequence, QIcon

try:
    from .device_item import DeviceItem
    from .passive_item import ResistorItem, CapacitorItem, _PassiveBase
...
from config.design_rules import (
    ...
    PASSIVE_RES_DEFAULT, PASSIVE_CAP_DEFAULT
)
...
class SymbolicEditor(QGraphicsView):
    ...
    device_orientation_changed = Signal(str, str) # dev_id, orientation
    edit_passive_requested = Signal(str)        # dev_id
    ...
    def load_placement(self, nodes, compact=False):
        ...
        for node in nodes:
            ...
            # Determine baseline dimensions based on device type
            if dtype == "res":
                def_w, def_h = PASSIVE_WIDTH_UM, PASSIVE_HEIGHT_UM
            elif dtype == "cap":
                def_w, def_h = PASSIVE_CAP_WIDTH_UM, PASSIVE_CAP_HEIGHT_UM
            else:
                def_w, def_h = 1.0, 0.5

            # Force standardized sizes for passives (ignoring JSON metadata overrides)
            if dtype == "res":
                width_um, height_um = PASSIVE_WIDTH_UM, PASSIVE_HEIGHT_UM
            elif dtype == "cap":
                width_um, height_um = PASSIVE_CAP_WIDTH_UM, PASSIVE_CAP_HEIGHT_UM
            else:
                width_um = geom.get("width", def_w)
                height_um = geom.get("height", def_h)

            width = width_um * self.scale_factor
            height = height_um * self.scale_factor
            ...
            if dtype == "res":
                ...
                val = node.get("electrical", {}).get("value") or node.get("electrical", {}).get("rval")
                item.set_value(val if val is not None else PASSIVE_RES_DEFAULT)
            elif dtype == "cap":
                item = CapacitorItem(node.get("id", "unknown"), x, y, width, height)
                val = node.get("electrical", {}).get("value") or node.get("electrical", {}).get("cval")
                item.set_value(val if val is not None else PASSIVE_CAP_DEFAULT)
            else:
                item = DeviceItem(
                    node.get("id", "unknown"),
                    dtype,
    ...
    def contextMenuEvent(self, event):
            ...
            if isinstance(it, DeviceItem):
                ...
                for dev_id, item in self.device_items.items():
                    if item is target_item:
                        ...
                        break
                break
            if type(it).__name__ in ("ResistorItem", "CapacitorItem") or isinstance(it, _PassiveBase):
                target_item = it
                for dev_id, item in self.device_items.items():
                    if item is target_item:
                        target_id = dev_id
        ...
        # Passive Value Edit
        act_edit_val = None
        if type(target_item).__name__ in ("ResistorItem", "CapacitorItem") or isinstance(target_item, _PassiveBase):
            act_edit_val = QAction("  Edit Passive Value", self)
            # Add a small icon if possible, but for now just text
            menu.addAction(act_edit_val)
            menu.addSeparator()

        # Abutment toggles (only if supported)
        act_left = act_right = act_clear = None
        if hasattr(target_item, "get_abut_left"):
        ...
        if chosen == act_edit_val and act_edit_val:
            print(f"DEBUG: Emitting edit_passive_requested for {target_id}")
            self.edit_passive_requested.emit(str(target_id))
        elif chosen == act_left and act_left:
            target_item.toggle_abut_left()
        elif chosen == act_right and act_right:
            target_item.toggle_abut_right()
        elif chosen == act_clear and act_clear:
            target_item.set_abut_left(False)
            target_item.set_abut_right(False)
        elif chosen == act_change_color:
            current_color = getattr(target_item, "_source_color", QColor("#4a90d9"))
            color = QColorDialog.getColor(current_color)
            if color.isValid():
                if hasattr(target_item, "set_custom_color"):
                    target_item.set_custom_color(color)
                # For passives, we might just update the brush if needed, but for now just try standard
                for item in self.scene.items():
                    name = getattr(target_item, "device_name", None)
                    if not name and hasattr(target_item, "get_logical_name"): name = target_item.get_logical_name()
                    
                    target_name = getattr(item, "device_name", None)
                    if not target_name and hasattr(item, "get_logical_name"): target_name = item.get_logical_name()
                    
                    if name and target_name == name and hasattr(item, "set_custom_color"):
                        item.set_custom_color(color)
        elif chosen == act_reset_color:
            if hasattr(target_item, "reset_custom_color"):
            ...
            for item in self.scene.items():
                ...
                if name and target_name == name and hasattr(item, "reset_custom_color"):
                    item.reset_custom_color()
