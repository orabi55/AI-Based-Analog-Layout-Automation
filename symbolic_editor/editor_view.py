...
try:
    ...
    from .passive_item import ResistorItem, CapacitorItem, InductorItem
...
class SymbolicEditor(QGraphicsView):
    ...
    def load_placement(self, nodes, compact=False):
        ...
        for node in nodes:
            ...
            is_passive = dtype in ("res", "cap", "ind")

            # Default dimensions: transistors use 1.0x0.5, passives use 1.0x0.6
            def_w = 1.0 if is_passive else 1.0
            def_h = 0.6 if is_passive else 0.5

            # Force standard dimensions for passives to ensure global uniformity, ignoring any JSON overrides.
            if is_passive:
                width = def_w * self.scale_factor
                height = def_h * self.scale_factor
            else:
                width = geom.get("width", def_w) * self.scale_factor
                height = geom.get("height", def_h) * self.scale_factor
            widths.append(width)
            ...
            e_val = node.get("electrical_value", 0.0)
            if dtype == "res":
                item = ResistorItem(node.get("id", "unknown"), x, y, width, height)
                item.electrical_value = e_val
                item.update_size_by_value(e_val if e_val > 0 else 1000.0)
            elif dtype == "cap":
                item = CapacitorItem(node.get("id", "unknown"), x, y, width, height)
                item.electrical_value = e_val
                item.update_size_by_value(e_val if e_val > 0 else 1e-12)
            elif dtype == "ind":
                item = InductorItem(node.get("id", "unknown"), x, y, width, height)
                item.electrical_value = e_val
                item.update_size_by_value(e_val if e_val > 0 else 1e-9)
            else:
                item = DeviceItem(
                    node.get("id", "unknown"),
                    dtype,
    ...
    def _on_hierarchy_drag_finished(self, group=None):
        """After moving a hierarchy group, sync positions back to node data."""
        if group is None:
            # Handle cases where the signal sender doesn't pass the group
            group = self.sender().parent() if hasattr(self.sender(), "parent") else None
    ...
    def contextMenuEvent(self, event):
        """Show a right-click menu to manually toggle abutment on a device or edit passive stats."""
        scene_pos = self.mapToScene(event.pos())
        items = self.scene.items(scene_pos)

        # Find the topmost DeviceItem or Passive Item under the cursor
        target_item = None
        target_id   = None
        for it in items:
            if isinstance(it, DeviceItem) or hasattr(it, 'electrical_value'):
                target_item = it
        ...
        if isinstance(target_item, DeviceItem):
    ...
    def trigger_passive_edit_by_id(self, dev_id):
        """Programmatically trigger the edit dialog for a specific device by ID."""
        item = self.device_items.get(dev_id)
        if item and hasattr(item, "electrical_value"):
            self.open_passive_edit_dialog(item)

    def open_passive_edit_dialog(self, target_item):
        """Open the SI-aware dialog to edit a passive component's value."""
        from PySide6.QtWidgets import QInputDialog, QMessageBox
        
        target_id = getattr(target_item, "device_name", "Device")
        # Formulate the initial pre-filled text (e.g., '1.00k')
        current_str = f"{target_item.electrical_value:g}"
        if hasattr(target_item, "_format_si") and target_item.electrical_value > 0:
            current_str = target_item._format_si(target_item.electrical_value, "").strip()

        text, ok = QInputDialog.getText(
            self, "Edit Passive Value", f"New value for {target_id} (e.g. 1k, 1M, 10m):",
            text=current_str
        )
        ...
        if ok and text.strip():
            ...
            # Clean optional trailing unit characters
            for unit_str in ['ohm', 'f', 'h', 'Ω', 'v', 'a']:
                if val_lower.endswith(unit_str):
                    val_text = val_text[:-len(unit_str)].strip()
                    val_lower = val_text.lower()
            ...
            try:
                parsed_val = None
                if val_lower.endswith('meg'):
                    parsed_val = float(val_text[:-3]) * 1e6
                else:
                    suffix = val_text[-1]
                    if suffix in scale_map:
                        parsed_val = float(val_text[:-1]) * scale_map[suffix]
                    elif suffix.lower() in scale_map and suffix.lower() != 'm':
                        parsed_val = float(val_text[:-1]) * scale_map[suffix.lower()]
                        
                if parsed_val is None:
                    parsed_val = float(val_text)
                    
                if parsed_val < 0:
                    QMessageBox.warning(self, "Invalid Value", "Electrical values must be positive.")
                    return
            except (ValueError, IndexError):
                QMessageBox.warning(self, "Invalid Value", f"'{text}' is not a valid number. Please use values like 1k, 1M, 10m.")
                return
            
            target_item.electrical_value = parsed_val
            dtype = getattr(target_item, "device_type", "")
            if dtype == "res":
                target_item.update_size_by_value(1000.0)
            elif dtype == "ind":
                target_item.update_size_by_value(1e-6)
            elif dtype == "cap":
                target_item.update_size_by_value(1e-9)
            
            # Re-select to trigger UI update
            was_selected = target_item.isSelected()
            if was_selected:
                target_item.setSelected(False)
                target_item.setSelected(True)

    def contextMenuEvent(self, event):
        """Provide device-specific actions (Rotate, Flip, Edit Value)."""
        scene_pos = self.mapToScene(event.pos())
        target_item = self.scene.itemAt(scene_pos, self.transform())
        
        target_id = getattr(target_item, "device_name", "Layout")
        ...
        if isinstance(target_item, DeviceItem):
            ...
            act_clear = QAction("Clear Both", self)
            menu.addAction(act_clear)
        elif hasattr(target_item, "electrical_value"):
            act_edit_val = QAction("Edit Passive Value", self)
            menu.addAction(act_edit_val)

        chosen = menu.exec(event.globalPos())

        if act_left and chosen == act_left:
            target_item.toggle_abut_left()
        elif act_right and chosen == act_right:
        ...
        elif act_edit_val and chosen == act_edit_val:
            self.open_passive_edit_dialog(target_item)
