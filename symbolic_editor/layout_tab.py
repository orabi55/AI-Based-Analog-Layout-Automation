...
try:
    ...
    from .dialogs.passive_edit_dialog import PassiveValueDialog
except ImportError:
    from chat_panel import ChatPanel
    from device_tree import DeviceTreePanel
    from editor_view import SymbolicEditor
...
class LayoutEditorTab(QWidget):
    ...
    def __init__(self, placement_file=None, parent=None):
        ...
        self.editor.edit_passive_requested.connect(self._on_edit_passive_value)
        self.properties_panel.property_changed.connect(self._on_property_edited)
    ...
    def _on_edit_passive_value(self, dev_id):
        print(f"DEBUG: _on_edit_passive_value triggered for {dev_id}")
        node = self._find_node(dev_id)
        if not node:
            print(f"DEBUG: Node {dev_id} not found in model!")
            return
        
        dev_type = node.get("type", "res")
        elec = node.get("electrical", {})
        
        # Robustly find current value
        current_val = elec.get("value")
        if current_val is None: current_val = elec.get("cval")
        if current_val is None: current_val = elec.get("rval")
        
        # Fallback to the visual item's value if not in the data model yet
        if current_val is None:
            item = self.editor.device_items.get(dev_id)
            if item and hasattr(item, "value"):
                current_val = item.value
                print(f"DEBUG: Found fallback current value from item: {current_val}")
        
        print(f"DEBUG: Opening dialog for {dev_id} with current_val={current_val}")
        
        dialog = PassiveValueDialog(dev_id, dev_type, current_val, self)
        if dialog.exec():
            new_val = dialog.result_value
            print(f"DEBUG: New value for {dev_id} will be {new_val}")
            
            # Update data model
            if "electrical" not in node:
                node["electrical"] = {}
            
            # Standardize on 'value' but keep 'cval'/'rval' for compatibility if they existed
            node["electrical"]["value"] = new_val
            if "cval" in elec: node["electrical"]["cval"] = new_val
            if "rval" in elec: node["electrical"]["rval"] = new_val
            
            # Refresh visual item
            item = self.editor.device_items.get(dev_id)
            if item:
                print(f"DEBUG: Updating visual item {dev_id}")
                if hasattr(item, "set_value"):
                    item.set_value(new_val)
                    print(f"DEBUG: set_value({new_val}) called on {dev_id}")
                else:
                    print(f"DEBUG: Item {dev_id} has no set_value method!")
            else:
                print(f"DEBUG: Visual item {dev_id} not found in editor!")
            
            # Also update properties panel if it's showing this device
            self._show_device_properties(dev_id)
    def _on_property_edited(self, dev_id, key, new_val):
        print(f"DEBUG: _on_property_edited triggered for {dev_id}: {key} = {new_val}")
        node = self._find_node(dev_id)
        if not node: return
        
        if "electrical" not in node:
            node["electrical"] = {}
        elec = node["electrical"]

        # 1. Handle value with SI parsing if it's the 'value' field
        if key == "value":
            from dialogs.passive_edit_dialog import PassiveValueDialog
            # Reuse the parser from our dialog
            parsed_val = PassiveValueDialog(dev_id, "res", 0)._parse_spice_value(new_val)
            print(f"DEBUG: Parsed {new_val} to {parsed_val}")
            
            elec["value"] = parsed_val
            if "cval" in elec: elec["cval"] = parsed_val
            if "rval" in elec: elec["rval"] = parsed_val
            
            # Refresh visual
            item = self.editor.device_items.get(dev_id)
            if item and hasattr(item, "set_value"):
                item.set_value(parsed_val)
        
        # 2. Handle other numeric fields (nf, m, etc.)
        else:
            try:
                # Try float first, then int if it looks like one
                num_val = float(new_val)
                if num_val == int(num_val): num_val = int(num_val)
                elec[key] = num_val
                
                # Refresh multiplier if changed
                if key == "m" or key == "nf":
                    # For transistors, we might need to reload placement or update nf
                    item = self.editor.device_items.get(dev_id)
                    if hasattr(item, 'nf'): item.nf = num_val
            except ValueError:
                elec[key] = new_val

        self.editor.scene.update()
        # self._push_undo(f"Edit {key}")
