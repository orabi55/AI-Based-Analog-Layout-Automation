...
class LayoutEditorTab(QWidget):
    ...
    stats_changed      = Signal(float, float, float) # res, ind, cap
    ...
    def __init__(self, placement_file=None, parent=None):
        ...
        self.device_tree.passive_value_edit_requested.connect(self.editor.trigger_passive_edit_by_id)
    ...
    def get_stats(self):
        return self._stat_res, self._stat_ind, self._stat_cap
        
    def set_stats(self, res, ind, cap):
        if self._stat_res != res or self._stat_ind != ind or self._stat_cap != cap:
            changed_res = self._stat_res != res
            changed_ind = self._stat_ind != ind
            changed_cap = self._stat_cap != cap
            ...
            devs = [s for s in selected if hasattr(s, "device_name") and hasattr(s, "electrical_value")]
            for d in devs:
                if changed_res and getattr(d, "device_type", "") == "res":
                    d.electrical_value = res
                    d.update_size_by_value()
                elif changed_ind and getattr(d, "device_type", "") == "ind":
                    d.electrical_value = ind
                    d.update_size_by_value()
                elif changed_cap and getattr(d, "device_type", "") == "cap":
                    d.electrical_value = cap
                    d.update_size_by_value()
            
            self.stats_changed.emit(res, ind, cap)
    ...
    def _on_selection_count_changed(self):
        ...
        devs = [s for s in selected if hasattr(s, "device_name") and hasattr(s, "electrical_value")]
        if len(devs) == 1:
            dtype = getattr(devs[0], "device_type", "")
            v = getattr(devs[0], "electrical_value", 0.0)
            if dtype in ("res", "ind", "cap"):
                self.stats_changed.emit(
                    v if dtype == "res" else self._stat_res,
                    v if dtype == "ind" else self._stat_ind,
                    v if dtype == "cap" else self._stat_cap
                )
    ...
    def _run_parser_pipeline(sp_path, oas_path="", abutment_enabled=True):
        ...
        for dev_name, dev in netlist.devices.items():
            dev_type = dev.type
            is_passive = dev_type in ("res", "cap", "ind")
            electrical = {
            ...
            # Map standard 'value' key from parser to 'electrical_value' for item scaling
            e_val = dev.params.get("value", 0.0)
            if not e_val and is_passive:
                 # fallback to specific keys if needed
                 if dev_type == "res": e_val = dev.params.get("r", 0.0)
                 elif dev_type == "cap": e_val = dev.params.get("c", 0.0)
                 elif dev_type == "ind": e_val = dev.params.get("l", 0.0)
            ...
            if layout_idx is not None and layout_idx < len(layout_instances):
                ...
                if is_passive:
                    # All passives (res, cap, ind) now use unified dimensions:
                    # width derived from length/fingers, height derived from width/0.1
                    width_um = max(raw_l * nf_p, PITCH_UM)
                    height_um = max(raw_w, 0.4)
            ...
            node_dict = {
                "id": dev_name, "type": dev_type, "electrical": electrical, 
                "geometry": geom, "electrical_value": e_val
            }
            if abut_info:
        ...
        if not device_mapping:
            ...
            for block_idx, inst_name in enumerate(block_order):
                ...
                passive_members = [n for n in members if n["type"] in ("res", "cap", "ind")]
                local_x = x_cursor
    ...
    def _compress_graph_for_storage(data):
        ...
        compressed = {
            ...
            "device_types": {
                ...
                "res": {"y_row": 1.630, "default_width": 0.294, "default_height": 0.4},
                "cap": {"y_row": 1.630, "default_width": 0.294, "default_height": 0.4},
                "ind": {"y_row": 1.630, "default_width": 0.294, "default_height": 0.4},
            },
            "devices": {},
