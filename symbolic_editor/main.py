...
class MainWindow(QMainWindow):
    ...
    def _new_tab(self, placement_file=None) -> LayoutEditorTab:
        ...
        tab.stats_changed.connect(self._sync_stats)
        ...
        self._sync_stats(*tab.get_stats())
        return tab
    ...
    def _sync_stats(self, res, ind, cap):
        self._ignore_stats_spin = True
        self._res_spin.setValue(res)
        self._ind_spin.setValue(ind)
        self._cap_spin.setValue(cap)
        self._ignore_stats_spin = False
    ...
    def _create_toolbar(self):
        ...
        lbl_res = QLabel(" RES:")
        lbl_res.setStyleSheet("color: #8899aa; font-family: 'Segoe UI'; font-size: 9pt;")
        tb.addWidget(lbl_res)
        self._res_spin = QDoubleSpinBox()
        self._res_spin.setRange(0.0, 1e15) # Positive only, expanded range
        self._res_spin.setDecimals(1)
        self._res_spin.setStyleSheet(spin_style)
        self._res_spin.valueChanged.connect(self._on_res_spin_changed)
        tb.addWidget(self._res_spin)

        lbl_ind = QLabel(" IND:")
        lbl_ind.setStyleSheet("color: #8899aa; font-family: 'Segoe UI'; font-size: 9pt;")
        tb.addWidget(lbl_ind)
        self._ind_spin = QDoubleSpinBox()
        self._ind_spin.setRange(0.0, 1e15) # Positive only
        self._ind_spin.setDecimals(1)
        self._ind_spin.setStyleSheet(spin_style)
        self._ind_spin.valueChanged.connect(self._on_ind_spin_changed)
        tb.addWidget(self._ind_spin)

        lbl_cap = QLabel(" CAP:")
        lbl_cap.setStyleSheet("color: #8899aa; font-family: 'Segoe UI'; font-size: 9pt;")
        tb.addWidget(lbl_cap)
        self._cap_spin = QDoubleSpinBox()
        self._cap_spin.setRange(0.0, 1e15) # Positive only
        self._cap_spin.setDecimals(1)
        self._cap_spin.setStyleSheet(spin_style)
        self._cap_spin.valueChanged.connect(self._on_cap_spin_changed)
        tb.addWidget(self._cap_spin)
    ...
    def _on_res_spin_changed(self, value):
        if hasattr(self, '_ignore_stats_spin') and self._ignore_stats_spin:
            return
        tab = self.current_tab()
        if tab:
            _, ind, cap = tab.get_stats()
            tab.set_stats(value, ind, cap)

    def _on_ind_spin_changed(self, value):
        if hasattr(self, '_ignore_stats_spin') and self._ignore_stats_spin:
            return
        tab = self.current_tab()
        if tab:
            res, _, cap = tab.get_stats()
            tab.set_stats(res, value, cap)

    def _on_cap_spin_changed(self, value):
        if hasattr(self, '_ignore_stats_spin') and self._ignore_stats_spin:
            return
        tab = self.current_tab()
        if tab:
            res, ind, _ = tab.get_stats()
            tab.set_stats(res, ind, value)
