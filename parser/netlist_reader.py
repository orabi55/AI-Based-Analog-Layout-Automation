...
def parse_value(value: str) -> float:
    """
    Convert values like 1u, 60n, 1m (milli), 1M (mega) -> float
    Case-sensitive: m=1e-3, M=1e6
    """
    value = value.strip()
    ...
    # Check multi-char lower-case suffixes like 'meg'
    v_lower = value.lower()
    if v_lower.endswith('meg'):
        return float(value[:-3]) * 1e6

    # Check case-sensitive single-char suffix
    suffix = value[-1]
    if suffix in scale:
        number_part = value[:-1]
        try:
    ...
    # Fallback to case-insensitive single-char for common units if not matched
    suffix_lower = suffix.lower()
    if suffix_lower in scale and suffix_lower != 'm': # don't fallback 'm' to 'M' or vice versa
        try:
            return float(value[:-1]) * scale[suffix_lower]
        except ValueError:
            pass
...
def parse_ind(tokens):
    ...
    params.setdefault('value', value)
    return Device(name, "ind", {"1": n1, "2": n2}, params)
...
def parse_line(line):
    ...
    # Inductor
    if tokens[0][0].upper() == 'L' and len(tokens) >= 4:
